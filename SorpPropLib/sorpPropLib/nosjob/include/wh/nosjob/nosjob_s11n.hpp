...
namespace s11n {
    ...
    */
    ...
    /**
       Overload to fix a broken(?) type lookup.
    */
    template <typename SerT>
    bool save( SerT const & ser, std::ostringstream & dest, uint8_t spacing = 0 )
    {
        return save<SerT>( ser, static_cast<std::ostream &>(dest), spacing);
    }
    ...
    */
    template <typename SerT>
    inline bool load( SerT & dest, std::istringstream & src )
    {
        return load<SerT>( dest, static_cast<std::istream &>(src));
    }
