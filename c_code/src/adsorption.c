...
Adsorption *newAdsorption(const char *isotherm_type) {
    // Try to allocate memory for Adsorption-structure
	//
    Adsorption *retAdsorption = (Adsorption *) malloc(sizeof(Adsorption));

    if (retAdsorption == NULL) {
		// Not enough memory available for Adsorption-structure
		//
		printf("\n\n###########\n# Warning #\n###########");
	...
	//
	if (strcmp(isotherm_type, "DualSiteSips") == 0) {
	...
	} else if (strcmp(isotherm_type, "DubininAstakhov") == 0) {
	...
	} else if (strcmp(isotherm_type, "DubininArctan1") == 0) {
		// Set function pointers to functions of Dubinin-Arctan1 isotherm
		//
		retAdsorption->vol_W_ARho = &adsorption_dubininArctan1_W_ARho;
		retAdsorption->vol_A_WRho = &adsorption_dubininArctan1_A_WRho;
		retAdsorption->vol_dW_dA_ARho = &adsorption_dubininArctan1_dW_dA_ARho;
		retAdsorption->vol_dA_dW_WRho = &adsorption_dubininArctan1_dA_dW_WRho;
 ...
 * --------------
 *
 * Destructor function to free allocated memory of an Adsorption-struct.
 *
 * Parameters:
 * -----------
 * 	struct *Adsorption:
 ...
 * -----------------
 *
 * Auxiliary function to throw warning if some function or parameter are
 * missing.
 *
 * Parameters:
 * -----------
...
double w_pT_surPsat(double p_Pa, double T_K, double isotherm_par[],
	...
	// Check if structures do exist and if functions are implemented
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "w_pT_surPsat");
	}

	// Calculate vapor pressure and saturated liquid of adsorpt
	//
	double p_sat_Pa = retRefrigerant->p_sat_T(T_K, p_sat_par, refrigerant);
...
double w_pT_vol(double p_Pa, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "w_pT_vol");
	}
...
double p_wT_surPsat(double w_kgkg, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "p_wT_surPsat");
	}
...
double p_wT_vol(double w_kgkg, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "p_wT_vol");
	}
	if (retRefrigerant->rho_l_T == NULL || rho_l_par == NULL) {
...
double T_pw_surPsat(double p_Pa, double w_kgkg, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "T_pw_surPsat");
	}
	if (retRefrigerant->dp_sat_dT == NULL) {
		return warning_func_par("dp_sat_dT", "T_pw_surPsat");
	}
	...
	return retAdsorption->sur_T_pwpsat(p_Pa, w_kgkg,
		retRefrigerant->p_sat_T,
		retRefrigerant->dp_sat_dT,
...
double T_pw_vol(double p_Pa, double w_kgkg, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "T_pw_vol");
	}
	if (retRefrigerant->rho_l_T == NULL || rho_l_par == NULL) {
		return warning_func_par("rho_l_T", "T_pw_vol");
	}
	if (retRefrigerant->dp_sat_dT == NULL) {
		return warning_func_par("dp_sat_dT", "T_pw_vol");
	}
...
double dw_dp_pT_surPsat(double p_Pa, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "dw_dp_pT_surPsat");
	}
...
double dw_dp_pT_vol(double p_Pa, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "dw_dp_pT_vol");
	}
	if (retRefrigerant->rho_l_T == NULL || rho_l_par == NULL) {
...
double dw_dT_pT_surPsat(double p_Pa, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "dw_dT_pT_surPsat");
	}
	if (retRefrigerant->dp_sat_dT == NULL) {
		return warning_func_par("dp_sat_dT", "dw_dT_pT_surPsat");
	}

	// Calculate vapor pressure and saturated liquid of adsorpt
	//
	double p_sat_Pa = retRefrigerant->p_sat_T(T_K, p_sat_par, refrigerant);
	double dp_sat_dT_PaK = retRefrigerant->dp_sat_dT(T_K, p_sat_par,
		refrigerant);
...
double dw_dT_pT_vol(double p_Pa, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "dw_dT_pT_vol");

	}
	if (retRefrigerant->dp_sat_dT == NULL) {
		return warning_func_par("dp_sat_dT", "dw_dT_pT_vol");
...
double dp_dw_wT_surPsat(double w_kgkg, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "dp_dw_wT_surPsat");
	}
...
double dp_dw_wT_vol(double w_kgkg, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "dp_dw_wT_vol");
	}
	if (retRefrigerant->rho_l_T == NULL || rho_l_par == NULL) {
...
double dp_dT_wT_surPsat(double w_kgkg, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "dp_dT_wT_surPsat");
	}
	if (retRefrigerant->dp_sat_dT == NULL) {
		return warning_func_par("dp_sat_dT", "dp_dT_wT_surPsat");
	}
...
double dp_dT_wT_vol(double w_kgkg, double T_K, double isotherm_par[],
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "dp_dT_wT_vol");
	}
	if (retRefrigerant->dp_sat_dT == NULL) {
		return warning_func_par("dp_sat_dT", "dp_dT_wT_vol");
	}
	if (retRefrigerant->rho_l_T == NULL || rho_l_par == NULL) {
 ...
 *	depending on pressure and temperature when volumetric approach (e.g.,
 *	Dubinin theory) is chosen as isotherm. Here, for surface approach, fluid
 *	properties at saturated state are not required and thus ignored.
 *
 * History:
 * --------
 *	01/13/2020, by Mirko Engelpracht:
 ...
 *	Wrapper function is required to calculate reduced spreading pressure
 *	depending on pressure and temperature when volumetric approach (e.g.,
 *	Dubinin theory) is chosen as isotherm. Here, for surface approach using
 *	saturation pressure, only saturation pressure is needed.
 *
 * History:
 * --------
 *	03/20/2020, by Mirko Engelpracht:
...
double piStar_pyxgTM_surPsat(double p_total_Pa, double y_molmol,
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "piStar_pyxgTM_surPsat");
	}
...
double piStar_pyxgTM_vol(double p_total_Pa, double y_molmol, double x_molmol,
	...
	//
	if (retRefrigerant->p_sat_T == NULL || p_sat_par == NULL) {
		return warning_func_par("p_sat_T", "piStar_pyxgTM_vol");
	}
	if (retRefrigerant->rho_l_T == NULL || rho_l_par == NULL) {
