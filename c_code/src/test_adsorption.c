...
int main() {
	...
	//
	Adsorption *isotherm_dss = newAdsorption("DualSiteSips");
	...
	double refrigerant_par_H2O[] = {647.096, 22064000, -7.85951783, 1,
		...
		1.80122502, 7.5, 0, 0};
	double satLiqDen_H2O_par[] = {647.096, 322, 1, 1, 0, 1.99274064,
	...
	// Define some input values for calculating equilibrium data
	//
	Refrigerant *refrigerant_water = newRefrigerant("VaporPressure_EoS1",
		"SaturatedLiquidDensity_EoS1", refrigerant_par_H2O);

	p_Pa = refrigerant_water->p_sat_T(283, refrigerant_par_H2O,
		refrigerant_water);
	T_K = 373;
	double p_sat_Pa = refrigerant_water->p_sat_T(T_K, refrigerant_par_H2O,
		refrigerant_water);
	...
	// Calculate equilibrium data
	//
	Adsorption *isotherm_dubininAstakhov = newAdsorption("DubininAstakhov");
	Refrigerant *refrigerant_r134a = newRefrigerant("VaporPressure_EoS1",
		"SaturatedLiquidDensity_EoS1", vapPre_par);

	p_sat_Pa = refrigerant_r134a->p_sat_T(T_K, vapPre_par,
		refrigerant_r134a);
	...
	T_K_inv = isotherm_dubininAstakhov->vol_T_pwpsatRho(p_Pa, w_kgkg,
		refrigerant_r134a->p_sat_T,
		refrigerant_r134a->rho_l_T,
		refrigerant_r134a->dp_sat_dT,
	...
	dw_dT_kgkgK = isotherm_dubininAstakhov->vol_dw_dT_pTpsatRho(p_Pa,
		T_K, p_sat_Pa, rho_kgm3, refrigerant_r134a->dp_sat_dT(T_K, vapPre_par,
	...
	dp_dT_Pakgkg = isotherm_dubininAstakhov->vol_dp_dT_wTpsatRho(w_kgkg, T_K,
		p_sat_Pa, rho_kgm3, refrigerant_r134a->dp_sat_dT(T_K, vapPre_par,
		refrigerant_r134a), refrigerant_r134a->drho_l_dT(T_K, satLiqDen_par),
		par_dubininAstakhov);
	 ...
	 *
	 */
	double par_dubininArctan1[] = {5.072313e-4, 2351.914097,
		-1526.9064, 4.128962e-6};
	...
	// Calculate equilibrium data
	//
	Adsorption *isotherm_dubininArctan1 = newAdsorption("DubininArctan1");

	p_sat_Pa = refrigerant_water->p_sat_T(T_K, refrigerant_par_H2O,
		refrigerant_water);
	rho_kgm3 = refrigerant_water->rho_l_T(T_K, satLiqDen_H2O_par);
	A_Jmol = 8.314462618f * T_K * log(p_sat_Pa/p_Pa);

	W = isotherm_dubininArctan1->vol_W_ARho(A_Jmol, rho_kgm3,
		par_dubininArctan1);
	A_Jmol_inv = isotherm_dubininArctan1->vol_A_WRho(W, rho_kgm3,
		par_dubininArctan1);

	w_kgkg = isotherm_dubininArctan1->vol_w_pTpsatRho(p_Pa, T_K, p_sat_Pa,
		rho_kgm3, par_dubininArctan1);
	p_Pa_inv = isotherm_dubininArctan1->vol_p_wTpsatRho(w_kgkg, T_K,
		p_sat_Pa, rho_kgm3, par_dubininArctan1);
	T_K_inv = isotherm_dubininArctan1->vol_T_pwpsatRho(p_Pa, w_kgkg,
		refrigerant_water->p_sat_T,
		refrigerant_water->rho_l_T,
		refrigerant_water->dp_sat_dT,
		refrigerant_water->drho_l_dT,
		par_dubininArctan1, refrigerant_par_H2O,
		satLiqDen_H2O_par, refrigerant_water);

	dW_dA = isotherm_dubininArctan1->vol_dW_dA_ARho(A_Jmol, rho_kgm3,
		par_dubininArctan1);
	dA_dW = isotherm_dubininArctan1->vol_dA_dW_WRho(W, rho_kgm3,
		par_dubininArctan1);
	dw_dp_kgkgPa = isotherm_dubininArctan1->vol_dw_dp_pTpsatRho(p_Pa,
		T_K, p_sat_Pa, rho_kgm3, par_dubininArctan1);
	dw_dT_kgkgK = isotherm_dubininArctan1->vol_dw_dT_pTpsatRho(p_Pa,
		T_K, p_sat_Pa, rho_kgm3, refrigerant_water->dp_sat_dT(T_K,
		refrigerant_par_H2O, refrigerant_water),
		refrigerant_water->drho_l_dT(T_K, satLiqDen_H2O_par),
		par_dubininArctan1);
	dp_dw_Pakgkg = isotherm_dubininArctan1->vol_dp_dw_wTpsatRho(w_kgkg, T_K,
		p_sat_Pa, rho_kgm3, par_dubininArctan1);
	dp_dT_Pakgkg = isotherm_dubininArctan1->vol_dp_dT_wTpsatRho(w_kgkg, T_K,
		p_sat_Pa, rho_kgm3, refrigerant_water->dp_sat_dT(T_K,
		refrigerant_par_H2O, refrigerant_water),
		refrigerant_water->drho_l_dT(T_K, satLiqDen_H2O_par),
		par_dubininArctan1);
	piStar_kgmol = isotherm_dubininArctan1->vol_piStar_pyxgTpsatRhoM(p_Pa, 1,
		1, 1, T_K, p_sat_Pa, rho_kgm3, 0.018015, par_dubininArctan1);

	// Print results of calculations
	//
	printf("\n\n##\n##\nCreated structure for Dubinin-Arctan1 isotherm.");
	...
	delAdsorption(isotherm_dubininArctan1);
