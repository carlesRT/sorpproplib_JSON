...
int main() {
	...
	printf("\n\nFor T = %f K and p = %f Pa, derivative of w with respect to p results in dw_dp = %f kg/kg/Pa.",
		T_K, p_Pa, dw_dp_kgkgPa);
	printf("\nFor T = %f K and p = %f Pa, derivative of w with respect to T results in dw_dT = %f kg/kg/K.",
		T_K, p_Pa, dw_dT_kgkgK);		
	printf("\nFor T = %f K and w = %f kg/kg, derivative of p with respect to w results in dp_dw = %f Pakg/kg.", 
		T_K, w_kgkg, dp_dw_Pakgkg);
	printf("\nFor T = %f K and w = %f kg/kg, derivative of p with respect to T results in dp_dT = %f Pa/K.", 
		T_K, w_kgkg, dp_dT_Pakgkg);
		
	printf("\n\nFor T = %f K and p = %f Pa, reduced spreading pressure results in piStar = %f mol/kg.",
		T_K, p_Pa, piStar_kgmol);
	...
	// Calculate equilibrium data
	//	
	Adsorption *isotherm_dubininAstakhov = newAdsorption("dubinin-astakov");
	...
	printf("\n\nFor A = %f J/mol, derivative of W with respect to A results in dW_dA = %f m3mol/J/kg.",
		A_Jmol, dW_dA);
	printf("\nFor W = %f m3/kg, derivative of A with respect to W results in dA_dW = %.3f Jkg/mol/m3.",
		W, dA_dW);
		
	printf("\n\nFor T = %f K and p = %f Pa, derivative of w with respect to p results in dw_dp = %f kg/kg/Pa.",
		T_K, p_Pa, dw_dp_kgkgPa);
	printf("\nFor T = %f K and p = %f Pa, derivative of w with respect to T results in dw_dT = %.4f kg/kg/K.",
		T_K, p_Pa, dw_dT_kgkgK);
	printf("\nFor T = %f K and w = %f kg/kg, derivative of p with respect to w results in dp_dw = %f Pakg/kg.", 
		T_K, w_kgkg, dp_dw_Pakgkg);
	printf("\nFor T = %f K and w = %f kg/kg, derivative of p with respect to T results in dp_dT = %f Pa/K.", 
