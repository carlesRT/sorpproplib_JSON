...
double absorption_activity_wangchao_g1_Txv1v2(double T_K, double x_molmol,
	...
	if (v1_m3mol < 0 || v2_m3mol < 0) {
		...
		//
		rho_21 = isotherm_par[4] / isotherm_par[3];
		rho_12 = isotherm_par[3] / isotherm_par[4];
	...
	//
	double Lambda_12 = rho_21 * exp(-isotherm_par[0] /
		(IDEAL_GAS_CONSTANT * T_K));
	double Lambda_21 = rho_12 * exp(-isotherm_par[1] /
		(IDEAL_GAS_CONSTANT * T_K));
	...
	double f1 = x_1/x_2 * exp(-isotherm_par[0] / (IDEAL_GAS_CONSTANT * T_K));
	double f2 = x_2/x_1 * exp(-isotherm_par[1] / (IDEAL_GAS_CONSTANT * T_K));
	double x_22 = 1 / (1 + f1);
	double x_12 = 1 - x_22;
	double x_11 = 1 / (1 + f2);
	double x_21 = 1 - x_11;
	...
	return exp(-log(aux1) + x_2 * (Lambda_21 / aux1 - Lambda_12 / aux2) +
		1 / (IDEAL_GAS_CONSTANT * T_K) * (isotherm_par[2] / 2) * (pow(x_21,
		2) * isotherm_par[1] + x_2 * x_22 * x_12 / x_1 * isotherm_par[0]));
}
...
double absorption_activity_wangchao_p_Txv1v2psat(double T_K, double x_molmol,
	...
	//
	double gamma = absorption_activity_wangchao_g1_Txv1v2(T_K, x_molmol,
...
double absorption_activity_wangchao_x_pTv1v2psat(double p_Pa, double T_K,
	...
	//
	for (p_guess_Pa = absorption_activity_wangchao_p_Txv1v2psat(T_K,
		...
		// temperature
		//
		p_guess_Pa = absorption_activity_wangchao_p_Txv1v2psat(T_K,
			x_guess_molmol, v1_m3mol, v2_m3mol, p_sat_Pa, isotherm_par);

		// Calculate derivative of the pressure with respect to molar fraction
		//
		dp_guess_dx_Pamolmol =
			(absorption_activity_wangchao_p_Txv1v2psat(T_K,
